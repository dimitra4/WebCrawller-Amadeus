...
import numpy as np
import pickle
from dbQueries import findUniqueCarrierCodeForDest
...
def change_to_datetime(col, train_data):
...
def predict(ml_model, dump, X_train, X_test, y_train, y_test):
...
def train():
    ...
    for i in ['flightDate', 'departureTime' ]:
        change_to_datetime(i, train_data)
    ...
    predict(RandomForestRegressor(n_estimators = 100),1,  X_train, X_test, y_train, y_test)
