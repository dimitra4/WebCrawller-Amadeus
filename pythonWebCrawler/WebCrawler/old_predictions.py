...
for i in ['flightDate','arrivalTime', 'departureTime' ]:
...
features = [ 'price',  'flightDate',  'arrival', 'departureTime_hours','departureTime_minutes', 'arrivalTime_hours', 
              'arrivalTime_minutes','duration_hours', 'duration_minutes']
...
print (cat_col)
#['departureIataCode', 'departureName', 'destinationIataCode', 'destinationName', 'arrival', 'carrierCode', 'duration', 'duration_hours', 'duration_minutes']

cont_col = [col for col in train_data.columns if train_data[col].dtype != 'O'] # continues data
print (cont_col)
#[ 'price', 'flightDate', 'numberOfFlight', 'totalStops', 'arrivalTime_hours', 'arrivalTime_minutes', 'departureTime_hours', 'departureTime_minutes']
...
final_train_data = pd.concat([categorical_data, airline, departure, destination, train_data[cont_col]], axis=1)
...
# print( mutual_info_classif(X,y_encoded) )
# imp = pd.DataFrame( mutual_info_classif(X,y_encoded), index=X.columns)
# imp.columns=['importance']
# imp.sort_values(by='importnace', ascending=False)
# #sorting values according to importance
